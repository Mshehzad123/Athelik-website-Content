...
function TrousersContent() {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          ...
          const filteredProducts = data.data.filter((product: Product) => {
            ...
            // Direct match for trousers
            if (productSubCategoryLower.includes('trouser') || productNameLower.includes('trouser') ||
                productSubCategoryLower.includes('pant') || productNameLower.includes('pant')) {
  ...
  return (
    <div className="min-h-screen">
      <Header />
      <main>
        <CollectionHero title="Trousers Collection" />
        {loading ? (
