...
function TwinsetsContent() {
      ...
      if (response.ok) {
        ...
        if (data.success && data.data) {
          ...
          const filteredProducts = data.data.filter((product: Product) => {
            ...
            // Direct match for twinsets
            if (productSubCategoryLower.includes('twin') || productNameLower.includes('twin') ||
                productSubCategoryLower.includes('set') || productNameLower.includes('set')) {
  ...
  return (
    <div className="min-h-screen">
      <Header />
      <main>
        <CollectionHero title="Twinsets Collection" />
        {loading ? (
